...
let sonos_room, sonos_http_api, reset_repeat, reset_shuffle, reset_crossfade;

try {
  var settings = JSON.parse(fs.readFileSync('usersettings.json', 'utf-8'));
  ({ sonos_room, sonos_http_api, reset_repeat, reset_shuffle, reset_crossfade } = settings);
} catch (error) {
  if (error.code === 'ENOENT') { // ENOENT is the error code for 'File not found'
    ...
    throw error; // If the error is not due to a missing file, rethrow it
  }
}
